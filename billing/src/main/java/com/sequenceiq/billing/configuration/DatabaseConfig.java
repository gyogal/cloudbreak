package com.sequenceiq.billing.configuration;
...
@ConditionalOnProperty(name = "billing.db.enabled", havingValue = "true", matchIfMissing = true)
public class DatabaseConfig {

    private static final String DEFAULT_SCHEMA_NAME = "public";

    @Value("${billing.db.env.user:}")
    private String dbUser;

    @Value("${billing.db.env.pass:}")
    private String dbPassword;

    @Value("${billing.db.env.db:}")
    private String dbName;

    @Value("${billing.db.env.poolsize:10}")
    private int poolSize;

    @Value("${billing.db.env.connectiontimeout:30}")
    private long connectionTimeout;

    @Value("${billing.db.env.minidle:2}")
    private int minimumIdle;

    @Value("${billing.db.env.idletimeout:10}")
    private long idleTimeout;

    @Value("${billing.db.env.schema:" + DEFAULT_SCHEMA_NAME + '}')
    private String dbSchemaName;

    @Value("${billing.db.env.ssl:}")
    private boolean ssl;

    @Value("#{'${billing.cert.dir:}/${billing.db.env.cert.file:}'}")
    private String certFile;

    @Value("${billing.hbm2d.strategy:validate}")
    private String hbm2ddlStrategy;

    @Value("${billing.hibernate.debug:false}")
    private boolean debug;

    @Value("${billing.hibernate.circuitbreaker:LOG}")
