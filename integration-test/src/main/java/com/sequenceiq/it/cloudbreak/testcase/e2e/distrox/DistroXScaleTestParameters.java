...
public class DistroXScaleTestParameters {
    ...
    public void setAdjustmentType(String adjustmentTypeName) {
        adjustmentType = Arrays.stream(AdjustmentType.values())
                .filter(adjustmentType -> adjustmentType.name().equalsIgnoreCase(adjustmentTypeName))
                .findFirst()
                .orElseThrow(() -> new TestFailException(String.format("There is no scaling AdjustmentType for '%s'",
                        adjustmentTypeName)));
    }
